...
export class EncryptionService {
  ...
  /**
   * 🔒 OPTIMIZADO: Hash rápido para login (menos iteraciones)
   */
  hashUserPasswordFast(password: string): string {
    // Usar menos iteraciones para login rápido
    return CryptoJS.PBKDF2(password, this.secretKey, {
      keySize: 256/32,
      iterations: 1000, // Reducido de 100000 a 1000 para velocidad
      hasher: CryptoJS.algo.SHA256
    }).toString();
  }
  ...
  hashUserPassword(password: string): string {
    // Para nuevos registros, usar hash seguro
    return CryptoJS.PBKDF2(password, this.secretKey, {
      keySize: 256/32,
      iterations: this.KEY_ITERATIONS,
      hasher: CryptoJS.algo.SHA256
  ...
  /**
   * 🔒 OPTIMIZADO: Verificación rápida de contraseñas
   */
  verifyPassword(password: string, hash: string): boolean {
    try {
      // Primero intentar con hash rápido
      const fastHash = this.hashUserPasswordFast(password);
      if (fastHash === hash) {
        return true;
      }

      // Si no coincide, intentar con hash completo (compatibilidad hacia atrás)
      const fullHash = this.hashUserPassword(password);
      const isValid = fullHash === hash;
      
      if (isValid) {
        console.log('🔍 Contraseña verificada con hash completo');
      }
