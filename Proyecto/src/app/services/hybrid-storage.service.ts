...
export class HybridStorageService {
  ...
  /**
   * Actualizar usuario existente (para cambios de PIN, etc.)
   */
  updateUser(updatedUser: any): { success: boolean; message: string } {
    try {
      ...
      localStorage.setItem(this.STORAGE_PREFIX + this.USER_KEY, JSON.stringify(updatedUser));
      ...
      if (userIndex !== -1) {
        ...
        localStorage.setItem(this.STORAGE_PREFIX + 'users', JSON.stringify(users));
        console.log('✅ HybridStorage: Usuario actualizado exitosamente');
        return { success: true, message: 'Usuario actualizado' };
      } else {
        console.warn('⚠️ HybridStorage: Usuario no encontrado en la lista');
        return { success: false, message: 'Usuario no encontrado' };
      }
    } catch (error) {
      console.error('❌ HybridStorage: Error actualizando usuario:', error);
